...
use dcap_qvl::verify::PreparedCollateral;
use zkdcap_core::DcapJournal;

pub fn main() {
    let quote: Vec<u8> = sp1_zkvm::io::read();
    let prepared: PreparedCollateral = sp1_zkvm::io::read();
    let now_secs: u64 = sp1_zkvm::io::read();

    let report = dcap_qvl::verify::sp1::verify_prepared(&quote, &prepared, now_secs)
